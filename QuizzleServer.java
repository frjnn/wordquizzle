...
/**
 * QuizzleServer is the core class of the Word Quizzle application. The
 * registration of new users is handled by providing a remote method
 * {@code registerUser}. Newly registered user are then inserted in the server's
 * database which is an instance of the class {@link QuizzleDatabase} thus
 * providing persistence of the users datas.
 * 
 * <p>
 * The QuizzleServer class is also provided with a threadpool {@code tPool} and
 * a selector {@code selector} in order to spawn a limited number of threads
 ...
 * provided with a {@link DatagramChannel} registered on the class selector
 * {@code selector}. QuizzleServer responds to the clients probe UDP packets
 ...
 * QuizzleClientGUI simultaneously.
 */
public class QuizzleServer extends UnicastRemoteObject implements RegistrationRMI {
    ...
    /**
     * The QuizzleDatabase. Initialized upon server creation. It first tries to
    ...
    /**
     * The data structure used by QuizzleServer to keep trace of online users. For
     * every logged user u the key is the server port number on which the connection
     * with the client of u is taking place and the value is u's nickname. It's also
     * used to check if the requested operation is legal during task execution.
     */
    private final ConcurrentHashMap<Integer, String> onlineUsers;

    /**
     * The data structure used by QuizzleServer to keep trace of online users IP
    ...
    /**
     * Creates a new QuizzleServer and initializes the dayabase, the threadpool, the
     ...
     * @throws RemoteException could be be thrown, QuizzleServer is a remote object.
     */
    public QuizzleServer(final int port, final int probe, final int length, final int invitation, final int words,
            final int thr) throws RemoteException {
        super();
        this.database = new QuizzleDatabase();
        this.threads = thr;
        // Notice the "+ 1" to include the Mailman thread.
        this.threadpool = new ThreadPoolExecutor(4, threads + 1, 1, TimeUnit.SECONDS,
                new LinkedBlockingQueue<Runnable>());
        this.postDepot = new LinkedBlockingQueue<QuizzleMail>();
        try {
        ...
        this.threadpool.execute(new Mailman(selector, postDepot));
        this.mainPort = port;
     ...
     * database and the calls {@code serialize()} on the server's QuizzleDatabase
     * instance.
     * 
    ...
    public static void main(final String[] args) throws RemoteException {
        ...
        if (args.length != 6 || args.length != 0 && args[0].equals("--help")) {
            System.out.println(
                    "Correct usage:\n\njava QuizzleServer <mainPort> <probePort> <match_timer> <invitation_timer> <num_words> <num_threads>");
            System.exit(1);
        }

        // Parsing main args.
        final int mainPort = Integer.parseInt(args[0]);
        final int probePort = Integer.parseInt(args[1]);
        ...
        if (mainPort <= 1024 || probePort <= 1024) {
        ...
        // Server creation.
        final QuizzleServer server = new QuizzleServer(mainPort, probePort, match_timer, invitation_timer, num_words,
                threads);
                    ...
                    while (keysIterator.hasNext()) {
                        ...
                        try {
                            ...
                            if (key.isAcceptable()) {
                                ...
                                final ServerSocketChannel QuizzleServer = (ServerSocketChannel) key.channel();
                                final SocketChannel qClient = QuizzleServer.accept();
                                System.out.println(
                                        "Accepted connection from client: " + server.getClientHostname(qClient));
                                qClient.configureBlocking(false);
                                // Registering the client's socket for read operations.
                                final SelectionKey keyClient = qClient.register(server.selector, SelectionKey.OP_READ);
                                ...
                                if (channel instanceof DatagramChannel) {
                                    ...
                                    // socket port.
                                    final DatagramChannel qProbeChannel = (DatagramChannel) key.channel();
                                    final ByteBuffer probeBuffer = ByteBuffer.allocate(28); // The dimension of an empty
                                                                                            // UDP
                                                                                            // datagram.
                                    final SocketAddress probeAddress = qProbeChannel.receive(probeBuffer);
                                    final String msg = String.valueOf(server.mainPort);
                                    try {
                                        final int n = qProbeChannel.send(ByteBuffer.wrap(msg.getBytes()), probeAddress);
                                    ...
                                    final SocketChannel qClient = (SocketChannel) key.channel();
                                    ...
                                    while ((k = qClient.read(bBuff)) != 0 && !crash) {
                                        ...
                                        if (k == -1) {
                                            ...
                                            crash = true;
                                            server.threadpool
                                                    .execute(new LogoutTask(server.onlineUsers, server.matchAddressBook,
                                                            server.selector, server.postDepot, key, true));
                                        }
                                    }
                                    if (crash)
                                        continue;
                                    ...
                                    case 0:
                                        ...
                                        final LoginTask logtsk = new LoginTask(server.database, server.onlineUsers,
                                                server.matchAddressBook, server.postDepot, key, nick, pwd, port);
                                        server.threadpool.execute(logtsk);
                                        break;
                                    case 1:
                                        // Logout Operation.
                                        final LogoutTask unlogtsk = new LogoutTask(server.onlineUsers,
                                                server.matchAddressBook, server.selector, server.postDepot, key, false);
                                    ...
                                    case 2:
                                        ...
                                        final AddFriendTask addtsk = new AddFriendTask(server.database,
                                                server.onlineUsers, server.postDepot, key, friend);
                                    ...
                                    case 3:
                                        ...
                                        final GetFriendListTask lsttsk = new GetFriendListTask(server.database,
                                                server.onlineUsers, server.postDepot, key);
