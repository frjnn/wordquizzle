...
/**
 * QuizzleClientGUI is the client program of WordQuizzle provided with a GUI.
 */
public class QuizzleClientGUI {
    ...
    /**
     * The user's nickname logged in on this QuizzleClientGUI instance.
     */
    ...
    private String login(final String nick, final String pwd) throws IOException {
        ...
        // QuizzleClientGUI class has just been created and the socket has been
        // initialized
        // but
        // is not connected.
        ...
        if (response.equals("Login error: wrong password.")) {
            // Closes the socket, if it's opne, because the login failed.
            if (!TCPSock.isClosed())
                TCPSock.close();
            TCPSock.close();
        }
        if (response.equals("Login successful.")) {
            // Assigns the name to this QuizzleClientGUI instance in order to recycle it for
            // subsequent requests.
            myName = nick;
        }
        return response;
        ...
        register.addActionListener(e -> {
            if (!nickname.getText().isEmpty() && !(password.getPassword().length == 0)) {
                ...
                if (esito.equals("Registration succeeded.")) {
                    ...
                    // Se username already taken.
                    JOptionPane.showMessageDialog(registrationFrame, "Username already taken.", "Word Quizzle - Error",
                            JOptionPane.INFORMATION_MESSAGE);
                }
            } else {
    ...
    /**
     * Where the QuizzleClientGUI's logic takes place.
     * 
     * @param args the main args.
     * @throws RemoteException      if something fishy happens during the remote
     *                              method invocation.
     * @throws IOException          if something fishy happens during the
     *                              QuizzleClientGUI instance initialization.
    ...
    public static void main(final String[] args) throws RemoteException, IOException, InterruptedException {
        ...
        if (args.length > 0) {
            ...
            System.out.println("Correct usage: java  QuizzleClientGUI.java <probe_port>");
            System.exit(0);
        }

        // Creates a new QuizzleClientGUI instance.
        final QuizzleClientGUI client = new QuizzleClientGUI(Integer.parseInt(args[0]));
